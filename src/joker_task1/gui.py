...
from .cli import build_predictions, map_at_k, tune_params
...
class Task1Gui:
    ...
    def _build_ui(self):
        ...
        def add_path_row(label: str, var: tk.StringVar, row: int):
            ...
            ttk.Entry(frm, textvariable=var, width=95).grid(row=row, column=1, sticky="we", padx=6)
        ...
        add_path_row("Qrels JSON (for tune/eval)", self.qrels_var, 2)
        add_path_row("Output prediction.json", self.output_var, 3)
        add_path_row("Output ZIP (optional)", self.zip_var, 4)
        add_path_row("Load tuned params JSON (optional)", self.params_in_var, 5)
        add_path_row("Save tuned params JSON (optional)", self.params_out_var, 6)

        ttk.Label(frm, text="Run ID").grid(row=7, column=0, sticky="w", pady=4)
        ttk.Entry(frm, textvariable=self.run_id_var, width=45).grid(row=7, column=1, sticky="w", padx=6)

        opts = ttk.Frame(frm)
        opts.grid(row=8, column=1, sticky="w", pady=8)
        ...
        btns = ttk.Frame(frm)
        btns.grid(row=9, column=1, sticky="w", pady=10)
        ...
        ttk.Label(frm, text="Prediction file to evaluate (for Eval button)").grid(row=10, column=0, sticky="w", pady=4)
        ttk.Entry(frm, textvariable=self.eval_pred_var, width=95).grid(row=10, column=1, sticky="we", padx=6)
        ttk.Button(frm, text="Browse", command=lambda: self._browse_file(self.eval_pred_var)).grid(row=10, column=2)

        self.progress = ttk.Progressbar(frm, orient="horizontal", mode="determinate", maximum=100)
        self.progress.grid(row=11, column=0, columnspan=3, sticky="we", pady=8)

        self.status_var = tk.StringVar(value="Idle")
        ttk.Label(frm, textvariable=self.status_var).grid(row=12, column=0, columnspan=3, sticky="w")
    ...
    def start_run(self):
        ...
        t = threading.Thread(target=self._worker_prediction, daemon=True)
        t.start()
    ...
    def start_eval_only(self):
        ...
        t = threading.Thread(target=self._worker_eval_only, daemon=True)
        t.start()
    ...
    def _worker_eval_only(self):
        try:
            ...
            score = self._evaluate_map(self.eval_pred_var.get(), self.qrels_var.get(), self.topk_var.get())
    ...
    def _worker_prediction(self):
        try:
            ...
            if params_in:
                self._emit("progress", f"Loading tuned params from {params_in}", 0.01)
                with Path(params_in).open("r", encoding="utf-8") as f:
                    params = json.load(f)
                self._emit("progress", f"Using loaded params: {params}", 0.03)

            if self.autotune_var.get():
                self._emit("progress", "Loading files for auto-tuning...", 0.05)
                ...
                if params_out:
                    ...
                    self._emit("progress", f"Saved tuned params to {params_out}", 0.58)
            ...
            if zip_path:
                ...
                self._emit("progress", f"Created zip: {zip_path}", 0.97)

            if self.eval_after_run_var.get() and qrels_path:
                self._emit("progress", "Running post-prediction evaluation...", 0.98)
                score = self._evaluate_map(output_path, qrels_path, self.topk_var.get())
                self._emit("progress", f"MAP@{self.topk_var.get()} on selected data: {score:.6f}", 0.995)
